...
void solve(sycl::queue &q, field *current, field *previous, double W, double K, double M, double Tm, double Lf, double Kt, double Cv, double dt){
    auto nx = current->nx;
    auto ny = current->ny;
    ...
    q.submit([&](sycl::handler &h){
        ...
        h.parallel_for(sycl::range(nx, ny), [=](sycl::id<2> idx){
            ...
            double dh_dphi =  6 * previous_acc[j][i] * (1 - previous_acc[j][i]);

            double dphi_dt =  M* ( K * Laplacian - d_freeEnergy - (Lf *((Temp_prev_acc[j][i]-Tm)/Tm))*dh_dphi);

            double dT_dt = (Kt * ((Temp_prev_acc[j][i+1] - 2 * Temp_prev_acc[j][i] + Temp_prev_acc[j][i-1]) / dx2
                                + (Temp_prev_acc[j+1][i] - 2 * Temp_prev_acc[j][i] + Temp_prev_acc[j-1][i]) / dy2)
                                +  Lf * dh_dphi * dphi_dt)/Cv;

            current_acc[j][i] = previous_acc[j][i] + dt * dphi_dt;
            Temp_curr_acc[j][i] = Temp_prev_acc[j][i] + dt * dT_dt;
        });
    });
