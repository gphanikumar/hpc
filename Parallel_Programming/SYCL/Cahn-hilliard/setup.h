...
void generate_field(field *Phase_field){
    ...
    Phase_field->laplacian.resize(newsize, 0.0);
    ...
    for(int i=0; i<Phase_field->nx + 2; ++i){
        Phase_field->data[i * (Phase_field->ny+2)] = Phase_field->data[i * (Phase_field->ny+2) + Phase_field->ny];
        Phase_field->data[i * (Phase_field->ny+2) + Phase_field->ny+1] = Phase_field->data[i * (Phase_field->ny+2) +1];
    }

    // Right boundary condition
    for(int j=0; j<Phase_field->ny+2; ++j){
        Phase_field->data[j] = Phase_field->data[(Phase_field->nx) * (Phase_field->ny + 2) +j];
        Phase_field->data[(Phase_field->nx + 1) * (Phase_field->ny + 2) +j] = Phase_field->data[(Phase_field->ny+2) +j];
    }
}
